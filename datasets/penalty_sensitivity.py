"""
Plot RWP statistics as a function of the hill-climbing penalty length scale,
with gradient threshold fixed at 0.02.
...
Usage:
    python datasets/penalty_sensitivity.py
"""
...
def main():
    ...
    print(f"  {len(ds['time'])} timesteps  (GT fixed at {BASE_KWARGS['edge_pruning_threshold']})\n")

    results = {}
    for pl in PENALTY_VALUES:
        kwargs = dict(BASE_KWARGS, penalty_length_scale_km=pl)
        print(f"penalty_length_scale_km = {pl:6d} ...", end=" ", flush=True)
    ...
    fig.suptitle(
        f"Sensitivity to Penalty Length Scale  (GT = {BASE_KWARGS['edge_pruning_threshold']})",
        fontsize=13,
    )
    ...
    for ax, key, ylabel, title in panels:
        y = [results[pl][key] for pl in x]
        ax.plot(x, y, "ko-", markersize=6)
        ax.axvline(2000, color="red", ls=":", alpha=0.5, label="current default (2000 km)")
        ax.set_xlabel("penalty_length_scale_km")
    ...
    plt.tight_layout()
    out_path = os.path.join(FIGURES_DIR, "penalty_sensitivity.png")
