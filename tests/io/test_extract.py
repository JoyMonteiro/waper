...
def test_extract_nodes(tracked_waper):
    ...
    assert {"time","rwp_id","node_id","node_type","lon","lat","scalar",
...
def test_extract_edges(tracked_waper):
    ...
    assert {"time","rwp_id","src_node_id","dst_node_id"}.issubset(df.columns)
    assert len(df) > 0

def test_extract_rwps(tracked_waper):
    df = extract_rwps(tracked_waper)
    assert {"time","rwp_id","weighted_lon","weighted_lat","peak_amp",
...
def test_extract_samples(tracked_waper):
    ...
    assert {"time","rwp_id","pt_idx","lon","lat"}.issubset(df.columns)
    assert len(df) > 0
...
def test_extract_track_tables(tracked_waper):
    ...
    assert {"time","feature","lon","lat","key"}.issubset(nodes.columns)
    assert {"src","dst","time_from","feat_from","time_to","feat_to",
